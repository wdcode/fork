...
public class FieldDictionary implements Caching {
    ...
    private void init() {
        ...
        if (JVM.isVersion(5))
            try {
                fieldUtil = (FieldUtil)JVM.loadClassForName("com.thoughtworks.xstream.converters.reflection.FieldUtil15", true).getDeclaredConstructor().newInstance();
            } catch (Exception e) {
                ;
            }
        if (fieldUtil == null)
    ...
    private DictionaryEntry buildDictionaryEntryForClass(final Class cls, final DictionaryEntry lastDictionaryEntry) {
        ...
        final Map keyedByFieldKey = new LinkedHashMap<>(lastDictionaryEntry.getKeyedByFieldKey());
