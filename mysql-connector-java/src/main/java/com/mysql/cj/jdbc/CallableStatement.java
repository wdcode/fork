...
public class CallableStatement extends ClientPreparedStatement implements java.sql.CallableStatement {
	...
	protected static class CallableStatementParam {
		...
		CallableStatementParam(String name, int idx, boolean in, boolean out, int jdbcType, String typeName,
				int precision, int scale, short nullability, int inOutModifier) {
			this.paramName = name;
			this.isIn = in;
			this.isOut = out;
			this.index = idx;
	...
	public class CallableStatementParamInfo implements ParameterMetaData {
		...
		/**
		 * Constructor that converts a full list of parameter metadata into one that only represents the placeholders
		 * present in the {CALL ()}.
		 * 
		 * @param fullParamInfo the metadata for all parameters for this stored procedure or function.
		 */
		...
		private void addParametersFromDBMD(java.sql.ResultSet paramTypesRs) throws SQLException {
			...
			while (paramTypesRs.next()) {
				...
				CallableStatementParam paramInfoToAdd = new CallableStatementParam(paramName, i++, isInParameter,
						isOutParameter, jdbcType, typeName, precision, scale, nullability, inOutModifier);
		...
		protected void checkBounds(int paramIndex) throws SQLException {
			...
			if ((paramIndex < 0) || (localParamIndex >= this.numParameters)) {
				throw SQLError.createSQLException(
						Messages.getString("CallableStatement.11", new Object[]{paramIndex, this.numParameters}),
		...
		public String getParameterClassName(int arg0) throws SQLException {
			...
			switch (mysqlType) {
				case YEAR:
					if (!CallableStatement.this.session.getPropertySet().getBooleanProperty(PropertyKey.yearIsDateType)
							.getValue()) {
	...
	private static String mangleParameterName(String origParameterName) {
		...
		StringBuilder paramNameBuf = new StringBuilder(
				PARAMETER_NAMESPACE_PREFIX.length() + origParameterName.length());
	...
	public CallableStatement(JdbcConnection conn, CallableStatementParamInfo paramInfo) throws SQLException {
		...
		this.noAccessToProcedureBodies = conn.getPropertySet().getBooleanProperty(PropertyKey.noAccessToProcedureBodies)
				.getValue();
	}
	...
	private void generateParameterMap() throws SQLException {
		synchronized (checkClosed().getConnectionMutex()) {
			...
			// if the user specified some parameters as literals, we need to provide a map from the specified
			// placeholders to the actual parameter numbers
			...
			if (this.paramInfo != null && q.getParameterCount() != parameterCountFromMetaData) {
				...
				if (startPos != -1) {
					...
					if (parenOpenPos != -1) {
						int parenClosePos = StringUtils.indexOfIgnoreCase(parenOpenPos, q.getOriginalSql(), ")", "'",
								"'", StringUtils.SEARCH_MODE__ALL);

						if (parenClosePos != -1) {
							List<?> parsedParameters = StringUtils.split(
									q.getOriginalSql().substring(parenOpenPos + 1, parenClosePos), ",", "'\"", "'\"",
									true);
			...
			if (this.callingStoredFunction) {
				if (paramIndex == 1) {
					...
					if (this.returnValueParam == null) {
						this.returnValueParam = new CallableStatementParam("", 0, false, true,
								MysqlType.VARCHAR.getJdbcType(), "VARCHAR", 0, 0,
	...
	/**
	 * Checks whether or not this statement is supposed to be providing streamable result sets...If output parameters
	 * are registered, the driver can not stream the results.
	 * 
	 * @throws SQLException if a database access error occurs
	 */
	private void checkStreamability() throws SQLException {
		if (this.hasOutputParams && createStreamingResultSet()) {
			throw SQLError.createSQLException(Messages.getString("CallableStatement.14"),
	...
	private void fakeParameterTypes(boolean isReallyProcedure) throws SQLException {
		synchronized (checkClosed().getConnectionMutex()) {
			...
			java.sql.ResultSet paramTypesRs = this.resultSetFactory.createFromResultsetRows(ResultSet.CONCUR_READ_ONLY,
	...
	private void determineParameterTypes() throws SQLException {
		synchronized (checkClosed().getConnectionMutex()) {
			...
			try {
				...
				List<?> parseList = StringUtils.splitDBdotName(procName, "", quotedId,
				...
				paramTypesRs = this.session.getPropertySet().<DatabaseTerm>getEnumProperty(PropertyKey.databaseTerm)
						.getValue() == DatabaseTerm.SCHEMA
								? dbmd.getProcedureColumns(null, useDb ? this.getCurrentDatabase() : tmpDb/* null */,
										procName, "%")
								: dbmd.getProcedureColumns(useDb ? this.getCurrentDatabase() : tmpDb/* null */, null,
										procName, "%");
	...
	public boolean execute() throws SQLException {
		synchronized (checkClosed().getConnectionMutex()) {
			...
			//
			// ResultSet metadata returns BIGINT for @com_mysql_jdbc_outparam_param_1 and
			// @com_mysql_jdbc_outparam_param_2
	...
	private String extractProcedureName() throws SQLException {
		String sanitizedSql = StringUtils.stripComments(((PreparedQuery<?>) this.query).getOriginalSql(), "`\"'",
		...
		throw SQLError.createSQLException(Messages.getString("CallableStatement.1"),
	...
	protected String fixParameterName(String paramNameIn) throws SQLException {
		synchronized (checkClosed().getConnectionMutex()) {
			...
			if (this.noAccessToProcedureBodies) {
				throw SQLError.createSQLException(Messages.getString("CallableStatement.23"),
						MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT, getExceptionInterceptor());
			}
	...
	protected int getNamedParamIndex(String paramName, boolean forOut) throws SQLException {
		synchronized (checkClosed().getConnectionMutex()) {
			...
			if ((paramName == null) || (paramName.length() == 0)) {
				throw SQLError.createSQLException(Messages.getString("CallableStatement.2"),
						MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT, getExceptionInterceptor());
			}
	...
	public Object getObject(int parameterIndex) throws SQLException {
		synchronized (checkClosed().getConnectionMutex()) {
			...
			Object retVal = rs.getObjectStoredProc(mapOutputParameterIndexToRsIndex(parameterIndex),
					paramDescriptor.desiredMysqlType.getJdbcType());
	...
	/**
	 * Returns the ResultSet that holds the output parameters, or throws an appropriate exception if none exist, or they
	 * weren't returned.
	 * 
	 * @param  paramIndex   parameter index
	 * @return              the ResultSet that holds the output parameters
	 * @throws SQLException if no output parameters were defined, or if no output parameters were returned.
	 */
		...
		synchronized (checkClosed().getConnectionMutex()) {
			...
			if (this.outputParameterResults == null) {
				if (this.paramInfo.numberOfParameters() == 0) {
					throw SQLError.createSQLException(Messages.getString("CallableStatement.7"),
							MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT, getExceptionInterceptor());
				}
				throw SQLError.createSQLException(Messages.getString("CallableStatement.8"),
						MysqlErrorNumbers.SQL_STATE_GENERAL_ERROR, getExceptionInterceptor());
			}
	...
	public void registerOutParameter(int parameterIndex, int sqlType) throws SQLException {
		try {
			...
			throw SQLError.createSQLFeatureNotSupportedException(
					Messages.getString("Statement.UnsupportedSQLType") + JDBCType.valueOf(sqlType),
					MysqlErrorNumbers.SQL_STATE_DRIVER_NOT_CAPABLE, getExceptionInterceptor());
		}
	}
			...
			if (numParameters > 0) {
				...
				if (hadOutputParams) {
					...
					try {
						...
						this.outputParameterResults = this.resultSetFactory.createFromResultsetRows(
								outParamRs.getConcurrency(), outParamRs.getType(),
								((com.mysql.cj.jdbc.result.ResultSetInternalMethods) outParamRs).getRows()); // note,
																												// doesn't
																												// work
																												// for
																												// updatable
			...
			if (this.paramInfo.numParameters > 0) {
				for (Iterator<CallableStatementParam> paramIter = this.paramInfo.iterator(); paramIter.hasNext();) {
					...
					if (inParamInfo.isOut && inParamInfo.isIn) {
						...
						try {
							setPstmt = ((Wrapper) this.connection.clientPrepareStatement(queryBuf.toString()))
									.unwrap(ClientPreparedStatement.class);

							if (((PreparedQuery<?>) this.query).getQueryBindings().getBindValues()[inParamInfo.index]
									.isNull()) {
								...
								if (parameterAsBytes != null) {
									if (parameterAsBytes.length > 8 && parameterAsBytes[0] == '_'
											&& parameterAsBytes[1] == 'b' && parameterAsBytes[2] == 'i'
											&& parameterAsBytes[3] == 'n' && parameterAsBytes[4] == 'a'
											&& parameterAsBytes[5] == 'r' && parameterAsBytes[6] == 'y'
											&& parameterAsBytes[7] == '\'') {
										...
										switch (inParamInfo.desiredMysqlType) {
											...
											case VARBINARY:
												...
												break;
											default:
												// the inherited PreparedStatement methods have already escaped and
												// quoted these parameters
				...
				for (Iterator<CallableStatementParam> paramIter = this.paramInfo.iterator(); paramIter.hasNext();) {
					...
					if (!this.callingStoredFunction && outParamInfo.isOut) {
						...
						if (this.placeholderToParameterIndexMap == null) {
							...
							if (!found) {
								throw SQLError.createSQLException(
										Messages.getString("CallableStatement.21",
												new Object[]{outParamInfo.paramName}),
										MysqlErrorNumbers.SQL_STATE_ILLEGAL_ARGUMENT, getExceptionInterceptor());
							}
						}

						this.setBytesNoEscapeNoQuotes(outParamIndex,
								StringUtils.getBytes(outParameterName, this.charEncoding));
					}
				}
			}
		}
		...
		synchronized (checkClosed().getConnectionMutex()) {
			...
			try {
				...
				if (procName.indexOf(".") != -1) {
					...
					procName = StringUtils
							.toString(StringUtils.stripEnclosure(StringUtils.getBytes(procName), "`", "`"));
				}
				ps = this.connection.prepareStatement(
						"SELECT SQL_DATA_ACCESS FROM information_schema.routines WHERE routine_schema = ? AND routine_name = ?");
				ps.setMaxRows(0);
				ps.setFetchSize(0);
