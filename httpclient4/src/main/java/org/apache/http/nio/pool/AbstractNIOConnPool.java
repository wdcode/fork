...
public abstract class AbstractNIOConnPool<T, C, E extends PoolEntry<T, C>>
	...
	/**
	 * @deprecated use
	 *             {@link AbstractNIOConnPool#AbstractNIOConnPool(ConnectingIOReactor, NIOConnFactory, SocketAddressResolver, int, int)}
	 */
	@Deprecated
	public AbstractNIOConnPool(final ConnectingIOReactor ioReactor, final NIOConnFactory<T, C> connFactory,
	 ...
	 */
	public AbstractNIOConnPool(final ConnectingIOReactor ioReactor, final NIOConnFactory<T, C> connFactory,
			final SocketAddressResolver<T> addressResolver, final int defaultMaxPerRoute, final int maxTotal) {
		super();
		Args.notNull(ioReactor, "I/O reactor");
		Args.notNull(connFactory, "Connection factory");
	...
	public Future<E> lease(final T route, final Object state, final long connectTimeout, final TimeUnit timeUnit,
	 ...
	 * @since 4.3
	 */
	public Future<E> lease(final T route, final Object state, final long connectTimeout, final long leaseTimeout,
			final TimeUnit timeUnit, final FutureCallback<E> callback) {
		...
		final LeaseRequest<T, C, E> leaseRequest = new LeaseRequest<T, C, E>(route, state,
				...
				leaseTimeout > 0 ? timeUnit.toMillis(leaseTimeout) : 0, future);
		this.lock.lock();
		try {
	...
	private boolean processPendingRequest(final LeaseRequest<T, C, E> request) {
		...
		if (pool.getAllocatedCount() < maxPerRoute) {
			...
			final SessionRequest sessionRequest = this.ioReactor.connect(remoteAddress, localAddress, route,
			...
			if (connectTimeout >= 0) {
				sessionRequest.setConnectTimeout(
						connectTimeout < Integer.MAX_VALUE ? (int) connectTimeout : Integer.MAX_VALUE);
			}
	...
	protected void requestCompleted(final SessionRequest request) {
		...
		final T route = (T) request.getAttachment();
		this.lock.lock();
		try {
	...
	public PoolStats getStats(final T route) {
		...
		try {
			...
			return new PoolStats(pool.getLeasedCount(), pendingCount + pool.getPendingCount(), pool.getAvailableCount(),
					getMax(route));
		} finally {
			this.lock.unlock();
		}
	...
	/**
	 * Use {@link #enumLeased(org.apache.http.pool.PoolEntryCallback)} or
	 * {@link #enumAvailable(org.apache.http.pool.PoolEntryCallback)} instead.
	 *
	 * @deprecated (4.3.2)
	 */
	@Deprecated
