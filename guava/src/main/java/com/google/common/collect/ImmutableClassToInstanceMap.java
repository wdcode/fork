...
/**
 * A {@link ClassToInstanceMap} whose contents will never change, with many other important properties detailed at
...
public final class ImmutableClassToInstanceMap<B> extends ForwardingMap<Class<? extends B>, B>
	 ...
	 * </pre>
	 * <p>
	 * After invoking {@link #build()} it is still possible to add more entries and build again. Thus each map generated
	 * by this builder will be a superset of any map generated before it.
	 *
	 * @since 2.0
	 */
	public static final class Builder<B> {
		...
		/**
		 * Associates {@code key} with {@code value} in the built map. Duplicate keys are not allowed, and will cause
		 * {@link #build} to fail.
		 */
		...
		/**
		 * Associates all of {@code map's} keys and values in the built map. Duplicate keys are not allowed, and will
	 ...
	 * duplicate keys (for example, if it is a {@code SortedMap} whose comparator is not <i>consistent with equals</i>),
...
//      ImmutableClassToInstanceMap.builder().putAll(map).build()
//      return cast;
//    }
